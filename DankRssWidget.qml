...
DesktopPluginComponent {
    ...
    function fetchFeed(index, collector) {
        ...
        Proc.runCommand("rssFetch:" + root.procTag + ":" + index, ["curl", "-sS", "--connect-timeout", "5", "--max-time", "10", "-L", "-A", "Mozilla/5.0 (X11; Linux x86_64) DankRssWidget/1.0", url], function(output, exitCode) {
            ...
            ListView {
                ...
                delegate: Rectangle {
                    ...
                    MouseArea {
                        ...
                        onClicked: {
                            if (root.sourceMode === 'miniflux') {
                                var currentStatus = (root.entryStatusMap[model.entryId] ?? {}).status;
                                var map = Object.assign({}, root.entryStatusMap);
