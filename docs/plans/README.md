...
Phase 3 (3b/3c/3d), accessible names, colour presets, OPML export, feed
autodiscovery, note images, snooze, mark-read-on-scroll, notification rules.
**Not built:** categories/folders, per-feed intervals, MPRIS enclosures,
Miniflux fast path.
...
| `2026-09-10-phase3b-summaries-design.md` | Per-article summaries (on-demand via AiProvider), and the digest/ranking measurements that superseded its latency argument | implemented — unreleased; one deviation, the feature toggle shipped global rather than per-instance, see its status block |
...
| `2026-09-08-phase3-ai-provider-design.md` | Local AI provider (OpenAI-compatible chat and embeddings) | 3a/3b/3c/3d all implemented — unreleased; 3c is `d` in the reader, 3d is off by default and unproven |
